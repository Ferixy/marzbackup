...
rm /root/marzbackup/crontabbackup.txt  >/dev/null 2>&1
crontab -l > /root/marzbackup/crontabbackup.txt
7z a -spf -p"$pass" -mhe=on -t7z -m0=lzma2 /root/marzbackup/MarzbanBackup.7z /etc/wireguard /opt/marzban /opt/marzban/.env /var/lib/marzban /etc/nginx /etc/haproxy /etc/sysctl.conf /root/marzbackup/crontabbackup.txt /var/lib/vnstat /opt/tblocker /etc/letsencrypt/live /etc/netplan
EOF
)
fi
