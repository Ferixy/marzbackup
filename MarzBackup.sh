...
7z a -spf -p"$pass" -mhe=on -t7z -m0=lzma2 /root/marzbackup/MarzbanBackup.7z /etc/wireguard /opt/marzban /opt/marzban/.env /var/lib/marzban /root/marzbackup/db-backup/* /etc/nginx /etc/haproxy /etc/sysctl.conf /root/marzbackup/crontabbackup.txt /var/lib/vnstat -x\!/var/lib/marzban/mysql 
